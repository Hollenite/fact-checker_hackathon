// background.js — with popup integration + live state tracking
// Replace <<<INSERT_PERPLEXITY_KEY_HERE>>> with your actual API key locally.

const API_KEY = "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";

let isChecking = false; // ✅ Tracks whether a fact-check is currently active
...
// ✅ Send messages to content + popup and manage active state
function sendFactCheckMessage(tabId, text, url) {
  isChecking = true; // now checking!
  chrome.tabs.sendMessage(tabId, { action: "showLoading" });
  chrome.runtime.sendMessage({ action: "showLoading" }); // notify popup
  ...
  (async () => {
    try {
      ...
      const response = await factCheckWithAI(text, contextText, url, apiKey);
      ...
      chrome.runtime.sendMessage({ action: "factCheckResult" }); // notify popup
    } catch (error) {
      ...
      chrome.runtime.sendMessage({ action: "factCheckError" }); // notify popup
    } finally {
      isChecking = false; // ✅ done
    }
  })();
}

// ✅ Allow popup to check current status on open
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "popupOpened") {
    sendResponse({ state: isChecking ? "checking" : "idle" });
  }
});
...
If you cannot find enough reliable sources to fact-check the statement, say so explicitly and explain why. If a claim is widely accepted as common knowledge, state this and provide general reference sources.`
  ...
  try {
    ...
    const result = await res.json().catch(err => {
      throw new Error('Invalid JSON from Perplexity API: ' + err.message);
    });

    console.log('Perplexity API response:', result);
    isChecking = false; // ✅ Mark as done once API returns
